...
class MyData(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        img = cv2.resize(img, dsize=(244, 244), interpolation=cv2.INTER_NEAREST)
        gt = cv2.resize(gt, dsize=(244, 244), interpolation=cv2.INTER_NEAREST)
...
class MyTestData(data.Dataset):
    ...
    def __init__(self, root, transform=True, ptag='piror'):
        super(MyTestData, self).__init__()
        self.root = root
        self._transform = transform

        img_root = os.path.join(self.root, 'images')
        map_root = os.path.join(self.root, 'piror')
