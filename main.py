...
parser.add_argument("--data",type=str,default="./ECSSD/")
...
if opt.phase == 'train':
    ...
    loader = torch.utils.data.DataLoader(
        MyData(dataRoot, transform=True), split='train', transform=True,
        batch_size=1, shuffle=True, num_workers=2, pin_memory=True)
else:
    outputRoot = opt.out
    loader = torch.utils.data.DataLoader(
        MyData(dataRoot, transform=True),
        batch_size=1, shuffle=True, num_workers=2, pin_memory=True)
...
if opt.phase=="train":
    for i in range(iter_num):
        for ib,data in enumerate(loader):
            ...
            if ib%2==0:
                print("loss: %.4f (epoch: %5d, step: %5d)"%(loss.data[0],i,ib))
            cnt+=1
            if cnt%1000==0:
                image=color_transform(outputs[0].cpu().max(0)[1].data)
                imsave('%s/FCN-epoch-%d-step-%d.png'%(checkRoot, i, ib),image)

        filename=('%s/FCN-epoch-%d-step-%d.pth'%(checkRoot, i, ib))
        torch.save(model.state_dict(), filename)
        print('save: (epoch: %d, step: %d)' % (i, ib))
else:
    for ib, data in enumerate(loader):
